...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/01/06 12:33:20 by ynacache          #+#    #+#             */
/*   Updated: 2018/01/07 16:26:35 by ynacache         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_conv_o_type(char **nbx, int *size_nb, t_arg *arg)
{
	...
	if (arg->arg.d >= 0)
		...
		(*nbx)[i] = bs[(((t_ull)arg->arg.d << (left + 1)) >> (nbbit - 2))];
	else if (arg->arg.d < 0 && ft_strchr("lL", ft_getheight(arg)))
		(*nbx)[i] = bs[(((t_ull)arg->arg.d << (left + 2)) >> (nbbit - 1))];
	else
		(*nbx)[i] = bs[(((t_ull)arg->arg.d << left) >> (nbbit - 1))];
